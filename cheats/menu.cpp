...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\necrophilex\\Scripts\\");
...
{
	const char* rage_sections[2] = { crypt_str("Globals"), crypt_str("Weapons") };
...
void c_menu::draw_ragebot(int child)
{
	if (!rage_section)
	{
		...
		else
		{
			...
			{
				if (g_cfg.misc.anti_untrusted == true)
				{
					ImGui::Text("Disable anti-untrusted for use exploits");
				}
				if (g_cfg.misc.anti_untrusted == false)
				{
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				draw_multicombo(crypt_str("Safe points conditions"), g_cfg.ragebot.weapon[hooks::rage_weapon].safe_points_conditions, safe_points_conditions, ARRAYSIZE(safe_points_conditions), preview);
				draw_combo(crypt_str("Body aim if"), g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaim_conditions, baim_cond, ARRAYSIZE(baim_cond));
				ImGui::Spacing();
				if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaim_conditions == 0)
				{
					ImGui::SliderInt(crypt_str("Baim when hp <="), &g_cfg.ragebot.weapon[hooks::rage_weapon].body_aim_health, 10, 100);
					ImGui::Spacing();
				}
...
{
	const char* aa_sections[1] = { crypt_str("AntiAim") };
	draw_semitabs(aa_sections, 1, antiaim_section, ImGui::GetStyle());
}
...
void c_menu::draw_antiaim(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##ANTIAIM1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			ImGui::Checkbox(crypt_str("Freestand"), &g_cfg.antiaim.freestand);
			draw_multicombo(crypt_str("Desync jitter condition"), g_cfg.antiaim.inverted_condition, condition_switch, ARRAYSIZE(condition_switch), preview);
	...
	{
		tab_start();
		ImGui::BeginChild(crypt_str("##FAKELAG2_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
		...
		draw_keybind(crypt_str("Auto peek"), &g_cfg.misc.automatic_peek, crypt_str("##AUTOPEEK__HOTKEY"));//никсер стайл, автопик в антиаимах
		ImGui::Spacing();
...
void c_menu::draw_visuals(int child)
{
	if (visuals_section != 2)
	{
		...
		//			ImGui::Checkbox(crypt_str("Custom hud"), &g_cfg.esp.custom_hud);
					...
					ImGui::Checkbox(crypt_str("Draw FOV"), &g_cfg.esp.draw_fov);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##allahmoment"), &g_cfg.esp.draw_fov_clr, ALPHA);
			...
			{
				child_title(crypt_str("Positions"));
		...
		else
		{
			...
			//		ImGui::Checkbox(crypt_str("Rainbow world"), &g_cfg.esp.rainbow);
			//		ImGui::Checkbox(crypt_str("Tyt pizdec"), &g_cfg.esp.pizdec);
		...
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (player == ENEMY || player == TEAM)
			{
				...
				if (player == ENEMY)
				{
					...
					ImGui::Checkbox(crypt_str("Hit chams"), &g_cfg.player.lag_hitbox);
					ImGui::SameLine();
			...
			if (g_cfg.player.type[player].chams[PLAYER_CHAMS_VISIBLE] || player == LOCAL && g_cfg.player.local_chams_type) //-V648
			{
				...
				else
				{
					...
					if (g_cfg.player.type[player].chams_type != 6)
					{
						...
						draw_combo(crypt_str("Player overlay type"), g_cfg.player.type[player].overlay_chams_type, overlay_cha_type, ARRAYSIZE(overlay_cha_type));
	...
	if (!child)
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Auto Accept"), &g_cfg.misc.auto_accept_matchmaking);
			...
			//	ImGui::Checkbox(crypt_str("Bypass sv_pure"), &g_cfg.misc.bypass);
				ImGui::Checkbox(crypt_str("Ad-block"), &g_cfg.misc.block_adv);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				//ну давай поговнокодим хули, все равно никто сурс не увидит
				auto width = 0;
				auto height = 0;
				...
				auto pizdec = height;
				//правда ведь никто не увидит? надеюсь да
				ImGui::SliderInt(crypt_str("Keybinds position x"), &g_cfg.esp.key_pos_x, 0, width);
				ImGui::SliderInt(crypt_str("Keybinds position y"), &g_cfg.esp.key_pos_y, 0, pizdec);

				ImGui::Text(crypt_str("Menu color"));
				ImGui::SameLine();
				ImGui::ColorEdit(("##color"), &g_cfg.menu.color, NOALPHA);
				...
				const char* channels_radio[] = { "None" , "Anison", "Radio record", "HardStyle", "BigFm", "BigFm deutsch rap", "Record Dubstep", "Record Dancecore" };
	...
	else
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Automatic strafes"), &g_cfg.misc.airstrafe);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				{
					draw_combo(crypt_str("Rifles"), g_cfg.misc.buybot1, mainwep, ARRAYSIZE(mainwep));
					padding(0, 3);
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(686 * dpi_scale, (child_height - 35) * dpi_scale));
	{
		...
		ImGui::PushItemWidth(520 * c_menu::get().dpi_scale);
		...
		if (ImGui::CustomButton(crypt_str("Open configs folder"), crypt_str("##CONFIG__FOLDER"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
		{
			std::string folder;
		...
		if (ImGui::CustomButton(crypt_str("Refresh configs"), crypt_str("##CONFIG__REFRESH"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
		{
			cfg_manager->config_files();
			files = cfg_manager->files;
		...
		if (ImGui::CustomButton(crypt_str("Create config"), crypt_str("##CONFIG__CREATE"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
		{
		...
		if (!next_save)
		{
			...
			if (ImGui::CustomButton(crypt_str("Save config"), crypt_str("##CONFIG__SAVE"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
			{
		...
		else
		{
			...
			if (ImGui::CustomButton(crypt_str("Are you sure?"), crypt_str("##AREYOUSURE__SAVE"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
			{
				save_config();
				prenext_save = true;
				clicked_sure = true;
		...
		if (ImGui::CustomButton(crypt_str("Load config"), crypt_str("##CONFIG__LOAD"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
			load_config();
		...
		if (!next_delete)
		{
			...
			if (ImGui::CustomButton(crypt_str("Remove config"), crypt_str("##CONFIG__delete"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
			{
		...
		else
		{
			...
			if (ImGui::CustomButton(crypt_str("Are you sure?"), crypt_str("##AREYOUSURE__delete"), ImVec2(582 * dpi_scale, 26 * dpi_scale)))
			{
				remove_config();
				prenext_delete = true;
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("necrophilex"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		ImGui::BeginChild(crypt_str("##tabs_area"), ImVec2(124 * dpi_scale, 509 * dpi_scale)); // there will be tabs
		{
			...
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("anti-hit"), super_ico, crypt_str("G"), active_tab == 1, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 1;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("legitbot"), super_ico, crypt_str("B"), active_tab == 6, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
			ImGui::Spacing();
			...
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("players"), super_ico, crypt_str("F"), active_tab == 3, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 3;
			ImGui::Spacing();
