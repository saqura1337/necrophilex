...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	else
	{
		...
		if (g_cfg.antiaim.type[type].desync)
		{
			...
			if (type != ANTIAIM_NORMAL && type != ANTIAIM_LEGIT || !g_cfg.antiaim.lby_type)
			{
				if (!flip)
	...
	{
		if (velocity <= 0.1f && g_ctx.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND)
			flip = invert_jitter;
	}
