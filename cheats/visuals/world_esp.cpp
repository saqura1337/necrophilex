...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	render::get().text(fonts[ESP], screen_origin.x, screen_origin.y - size.y * 0.5f, Color(220, 220, 220, 245), HFONT_CENTERED_X | HFONT_CENTERED_Y, "%i FT", (int)distance);
}
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	render::get().circle_filled(screen_origin.x, screen_origin.y - 17.0f, 18, 20, Color(15, 15, 15, 187));
	render::get().CircularProgressBar(screen_origin.x, screen_origin.y - 17.0f, 20, 18, 40, size.x * factor, g_cfg.esp.smoke_timer_color, false);

	static auto alpha = 2.0f;
	render::get().Draw3DCircle(origin, 144.0f, g_cfg.esp.smoke_timer_color);

	render::get().text(fonts[WARNING_icon], screen_origin.x + 1.0f, screen_origin.y - 19.0f, g_cfg.esp.smoke_timer_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "k");
