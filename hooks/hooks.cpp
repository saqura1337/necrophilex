...
namespace hooks
{
	...
	{
		ImGui::CreateContext();
		if (yourlogo == nullptr) D3DXCreateTextureFromFileInMemoryEx(pDevice, &logobyte, sizeof(logobyte), 65, 60, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &yourlogo);
		...
		// Menu fonts
		c_menu::get().neverpuk = io.Fonts->AddFontFromMemoryTTF((void*)neverpuk, sizeof(neverpuk), 15.f, &m_config, ranges);
		c_menu::get().neverpuk_small = io.Fonts->AddFontFromMemoryTTF((void*)neverpuk, sizeof(neverpuk), 13.f, &m_config, ranges);
		c_menu::get().neverpuk_large = io.Fonts->AddFontFromMemoryTTF((void*)neverpuk, sizeof(neverpuk), 20.f, &m_config, ranges);
		c_menu::get().super_ico = io.Fonts->AddFontFromMemoryTTF((void*)super_ico, sizeof(super_ico), 25.f, &m_config, ranges);
...
//bool _fastcall hooks::send_net_message(void* ecx, void* edx, INetMessage* msg, bool reliable, bool voice)
//{
//	if (g_cfg.misc.bypass)
//		return;
//
//	static auto eventt = hk_netchannel->get_func_address<sendnetmsg_fn>(40);
//
//	if (!msg)
...
//
//
//	return eventt(ecx, msg, reliable, voice);
//}
