...
#include "..\..\cheats\legitbot\legit_bt.h"
...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	if (gameoverlay_return_address != (uintptr_t)(_ReturnAddress()) && g_cfg.misc.obs)
		return hooked_sceneend;
	misc::get().break_prediction(m_pcmd);
	misc::get().SlideWalk(m_pcmd);
	misc::get().lsd();
	misc::get().knifelefthand();
...
//	lagcompensation::get().player_resolver->StoreAntifreestand();
	...
	if (g_ctx.globals.loaded_script)
		for (auto current : c_lua::get().hooks.getHooks(crypt_str("on_createmove")))
			current.func();
	// pitch 0
	if (g_ctx.local()->get_animation_state()->m_bInHitGroundAnimation)
	{
		if (g_ctx.local()->get_animation_state()->m_flHeadHeightOrOffsetFromHittingGroundAnimation)
		{
			local_animations::get().local_data.fake_angles.x = 0.0f;
			local_animations::get().local_data.real_angles.x = 0.0f;
			local_animations::get().local_data.stored_real_angles.x = 0.0f;
		}
	}
	...
	if (g_cfg.misc.buybot_enable && g_ctx.globals.should_buy)
	{
		...
		if (!g_ctx.globals.should_buy)
		{
			...
			case 5:
				...
				break;
			case 6:
				buy += crypt_str("buy aug; buy sg556; ");
				break;
			}
