...
IMaterial* CreateMaterial(bool lit, const std::string& material_data)
{
	...
	auto matname = crypt_str("necrophilex_") + std::to_string(created);
	++created;
...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	if (is_player)
	{
		...
		if (type == ENEMY)
		{
			...
			auto double_material = materials[g_cfg.player.type[ENEMY].overlay_chams_type];
		...
		else if (type == TEAM)
		{
			...
			auto double_material = materials[g_cfg.player.type[TEAM].overlay_chams_type];;
		...
		else if (m_input()->m_fCameraInThirdPerson)
		{
			...
			auto double_material = materials[g_cfg.player.type[LOCAL].overlay_chams_type];;
