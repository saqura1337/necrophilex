...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && g_ctx.globals.scoped && !g_cfg.esp.custom_scopes_line && !g_cfg.esp.dynamic_scopes_line)
			{
				static int w, h;
	...
	//	otheresp::get().keybinds();
	//	otheresp::get().get_keys();
