...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.enable_key, key_bind(HOLD), crypt_str("Ragebot.enable_key"));
	...
	setup_item(&g_cfg.ragebot.dt_teleport, false, crypt_str("Ragebot.dt_teleport"));
	...
	setup_item(&g_cfg.ragebot.head_aim_key, key_bind(HOLD), crypt_str("Ragebot.head_aim_key"));
	setup_item(&g_cfg.ragebot.pitch_correction, false, crypt_str("Ragebot.pitch_correction"));
	setup_item(&g_cfg.ragebot.dt_opt, 4, crypt_str("Ragebot.dt_opt"));
	setup_item(&g_cfg.ragebot.rage_aimbot_ignore_limbs, false, crypt_str("Ragebot.aimbot_ignore_limbs"));
	setup_item(&g_cfg.ragebot.remove_recoil, false, crypt_str("Ragebot.remove_recoil"));
	setup_item(&g_cfg.ragebot.enabled_weapon, false, crypt_str("Ragebot.enabled_weapon"));

	setup_item(&g_cfg.ragebot.resolver, false, crypt_str("Ragebot.desync_resolver"));

	setup_item(&g_cfg.ragebot.resolver_override_key, key_bind(TOGGLE), crypt_str("Ragebot.resolver_override_key"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].safe_points_conditions, 4, std::to_string(i) + crypt_str("Ragebot.safe_points_conditions"));
		setup_item(&g_cfg.ragebot.weapon[i].accuracy_boost, false, std::to_string(i) + crypt_str("Ragebot.accuracy_boost"));
		setup_item(&g_cfg.ragebot.weapon[i].accuracy_boost_modifiers, 3, crypt_str("Ragebot.accuracy_boost_modifiers"));
		setup_item(&g_cfg.ragebot.dt_mode, 0, crypt_str("Ragebot.dt_mode"));
		setup_item(&g_cfg.ragebot.dt_tickbase_shift, 13, crypt_str("Ragebot.dt_tickbase_shift"));

		/*setup_item(&g_cfg.ragebot.weapon[i].double_tap_speed, false, std::to_string(i) + crypt_str("Ragebot.double_tap_speed"));
		setup_item(&g_cfg.ragebot.dt_mode, 0, crypt_str("Ragebot.dtmode"));
		setup_item(&g_cfg.ragebot.dt_shift, 0, crypt_str("Ragebot.dt_shift"));
		setup_item(&g_cfg.ragebot.dt_tolerance, 0, crypt_str("Ragebot.dt_tolerance"));*/
	}
	...
	setup_item(&g_cfg.ragebot.twoshot, false, crypt_str("Ragebot.twoshot"));
	...
	for (auto i = 0; i < 4; i++)
	{
		...
		setup_item(&g_cfg.antiaim.type[i].spin_speed, 1, std::to_string(i) + crypt_str("Antiaim.spin_speed"));
		setup_item(&g_cfg.antiaim.type[i].desync_spin_range, 60, std::to_string(i) + crypt_str("Antiaim.desync_spin_range"));
		setup_item(&g_cfg.antiaim.type[i].inverted_desync_spin_range, 60, std::to_string(i) + crypt_str("Antiaim.inverted_desync_spin_range"));
	}
	...
	setup_item(&g_cfg.player.arrows_color, Color(255, 255, 255, 123), crypt_str("Player.arrows_color"));
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		setup_item(&g_cfg.player.type[i].glow_color, Color(255, 0, 0), std::to_string(i) + crypt_str("Player.glow_color"));
		setup_item(&g_cfg.player.type[i].chams, 2, std::to_string(i) + crypt_str("Player.chams"));
		setup_item(&g_cfg.player.type[i].chams_color, Color(124, 252, 0), std::to_string(i) + crypt_str("Player.chams_color"));
		setup_item(&g_cfg.player.type[i].xqz_color, Color(30, 144, 255), std::to_string(i) + crypt_str("Player.xqz_color"));
		setup_item(&g_cfg.player.type[i].chams_type, 0, std::to_string(i) + crypt_str("Player.chams_type"));
		setup_item(&g_cfg.player.type[i].overlay_chams_type, 0, std::to_string(i) + crypt_str("Player.overlay_chams_type"));
...
//	setup_item(&g_cfg.esp.custom_hud, false, crypt_str("Esp.custom_hud"));
	setup_item(&g_cfg.esp.Color_KeyBind, Color(255, 255, 255, 240), crypt_str("Menu.keybind_color"));
	setup_item(&g_cfg.esp.fix_zoom_sensivity, false, crypt_str("Esp.fix_zoom_sensivity"));
	setup_item(&g_cfg.misc.break_prediction, false, crypt_str("Misc.break_prediction"));
	setup_item(&g_cfg.misc.lsd, false, crypt_str("Misc.lsd"));
	setup_item(&g_cfg.misc.leftknife, false, crypt_str("Misc.leftknife"));
	setup_item(&g_cfg.misc.keybinds, false, crypt_str("Misc.keybinds"));
	...
	setup_item(&g_cfg.esp.rainbow, false, crypt_str("Esp.rainbow"));
	setup_item(&g_cfg.esp.pizdec, false, crypt_str("Esp.pizdec"));
	...
	setup_item(&g_cfg.esp.key_pos_x, 0, crypt_str("Esp.key_pos_x"));
	setup_item(&g_cfg.esp.key_pos_y, 0, crypt_str("Esp.key_pos_y"));
	setup_item(&g_cfg.esp.client_bullet_impacts, false, crypt_str("Esp.client_bullet_impacts"));
	setup_item(&g_cfg.esp.client_bullet_impacts_color, Color(255, 0, 0), crypt_str("Esp.client_bullet_impacts_color"));
	setup_item(&g_cfg.esp.server_bullet_impacts, false, crypt_str("Esp.server_bullet_impacts"));
	setup_item(&g_cfg.esp.server_bullet_impacts_color, Color(0, 0, 255), crypt_str("Esp.server_bullet_impacts_color"));
	...
	setup_item(&g_cfg.esp.p2c, false, crypt_str("Esp.penetration_reticle_2"));
	setup_item(&g_cfg.esp.penetration_reticle, false, crypt_str("Esp.penetration_reticle"));
	setup_item(&g_cfg.esp.penetration_reticle_mode, false, crypt_str("Esp.penetration_reticle_mode"));
	...
	setup_item(&g_cfg.esp.zeus_color, Color(255, 255, 255), crypt_str("Esp.taser_color"));
	setup_item(&g_cfg.misc.radio, false, crypt_str("Misc.radio"));
	setup_item(&g_cfg.misc.radiochannel, 0, crypt_str("Misc.radiochannel"));
	setup_item(&g_cfg.misc.radiovolume, 100.f, crypt_str("Misc.radiovolume"));
	setup_item(&g_cfg.misc.spectators_list, false, crypt_str("Misc.spectators_list"));
	setup_item(&g_cfg.misc.spectatorlist_pos_y, false, crypt_str("Esp.spectators_list_pos_y"));
	setup_item(&g_cfg.misc.spectatorlist_pos_x, false, crypt_str("Esp.spectators_list_pos_x"));

	setup_item(&g_cfg.misc.Color_spectator, Color(173, 255, 47, 240), crypt_str("Menu.spectator_color"));
	...
	setup_item(&g_cfg.misc.Color_spectator, Color(173, 255, 47, 240), crypt_str("Menu.utomatic_peek_color"));
	...
	setup_item(&g_cfg.misc.warning_mode, 0, crypt_str("Misc.warning_mode"));
	setup_item(&g_cfg.warning.trace.enable, false, crypt_str("warning.hndfhgsd"));
	setup_item(&g_cfg.warning.trace.color, Color(25, 55, 240), crypt_str("warning.ccad"));
	setup_item(&g_cfg.warning.trace.type, 0, crypt_str("warning.iuosadsafsdafsasdaffdsay"));
	setup_item(&g_cfg.warning.trace.width, 1.4f, crypt_str("warning.szxczxcbbbbbbbbbbxzxczczxc13132"));
	setup_item(&g_cfg.warning.trace.visible_only, false, crypt_str("warning.hhhghgffhggfhgfhgfhd"));

	setup_item(&g_cfg.warning.main.enable, false, crypt_str("warning.qeqewq"));
	setup_item(&g_cfg.warning.main.distance_warning, false, crypt_str("warning.zxc"));
	setup_item(&g_cfg.warning.main.d_warn_type, 1, crypt_str("warning.iuoy"));
	setup_item(&g_cfg.warning.main.visible_only, false, crypt_str("warning.hhjgfh"));

	setup_item(&g_cfg.warning.main.show_bg, true, crypt_str("warning.213"));
	setup_item(&g_cfg.warning.main.show_damage_dist, false, crypt_str("warning.d f"));
	setup_item(&g_cfg.warning.main.show_icon, 1, crypt_str("warning.z"));
	setup_item(&g_cfg.warning.main.show_timer, true, crypt_str("warning.asd"));
	setup_item(&g_cfg.warning.main.color_by_time, true, crypt_str("warning.warn123_col"));

	setup_item(&g_cfg.warning.main.bg_col, Color(21, 21, 21), crypt_str("warning.bg"));
	setup_item(&g_cfg.warning.main.icon_col, Color(255, 255, 255), crypt_str("warning.icon"));
	setup_item(&g_cfg.warning.main.timer_col, Color(255, 255, 255), crypt_str("warning.timer_col"));
	setup_item(&g_cfg.warning.main.warn_col, Color(220, 90, 5), crypt_str("warning.warn_col"));

	setup_item(&g_cfg.misc.obs, false, crypt_str("Misc.obs"));
	setup_item(&g_cfg.misc.zeusparty, false, crypt_str("Misc.zeus_party"));
	setup_item(&g_cfg.misc.immunity, false, crypt_str("Misc.force_immunity"));

		setup_item(&g_cfg.misc.disablepanoramablur, false, crypt_str("Misc.disablepanoramablur"));

	setup_item(&g_cfg.misc.noduck, false, crypt_str("Misc.noduck"));
	setup_item(&g_cfg.misc.slowwalk, false, crypt_str("Misc.slowwalk"));
	...
	setup_item(&g_cfg.misc.xyina360, false, crypt_str("Misc.xyina360"));
	...
	setup_item(&g_cfg.misc.logs_mode, 3, crypt_str("Misc.logs_mode"));
	...
	setup_item(&g_cfg.misc.log_color2, Color(255, 255, 255), crypt_str("Misc.log_color2"));
	...
	setup_item(&g_cfg.menu.color, Color(255, 255, 255, 255), crypt_str("Menu.menu_color"));
	setup_item(&g_cfg.menu.watermark, true, crypt_str("Menu.watermark"));
	setup_item(&g_cfg.menu.Color_watermark, Color(255, 255, 255, 0), crypt_str("Menu.watermark_color"));

	setup_item(&g_cfg.menu.panic_key, key_bind(HOLD), crypt_str("Misc.panic_key"));
